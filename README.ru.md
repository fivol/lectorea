...
следующему курсу, за который реально можно взяться. Сверху профиль открывается
цифрами: часы просмотра, лекции и курсы позади, дни подряд и последние четыре
недели полосой дней, а под ними путь ко всему избранному.

**Прогресс с точностью до лекции.** Смотрите во встроенном плеере — он запомнит
секунду, на которой вы остановились, и профиль откроется карточкой
**Продолжить**, которая вернёт вас туда же. У лекций, просмотренных на YouTube,
есть своя галочка, shift отмечает подряд, а одна кнопка — весь плейлист. Курс
сам считает себя начатым с первой лекции и пройденным, когда плейлист позади, —
пока вы не скажете иначе. Подробнее:
[docs/interface.md](docs/interface.md#progress-down-to-the-lecture).

![Профиль](docs/images/profile.webp)

**Одна комната с полками вместо четырёх вкладок.** Что изучаете сейчас, к чему
идёте, какие плейлисты сохранили, что открывали недавно и что уже позади —
каждая полка показывает несколько карточек и раскрывается целиком, не выходя из
профиля.
...
- **«Предложить плейлист»** у пустого курса открывает готовую форму. В ней одно
  поле — ссылка. Вуз, преподаватель, язык и продолжительность подтягиваются
  с YouTube сами.
