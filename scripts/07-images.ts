...
async function main(): Promise<void> {
  ...
  if (!useOpenAI) {
    ...
    for (const course of sources.courses) {
      ...
      fs.writeFileSync(path.join(paths.courseImages, `${course.id}.svg`), svg, 'utf8');
      written += 1;
    }
    console.log(`✓ data:images: ${written} procedural SVGs (seed "${visual.seedSalt}")`);
  ...
  const targets = sources.domains.filter((domain) => !only || only.includes(domain.id));
  console.log(`· generating ${targets.length} domain images through ${MODELS.image}`);
  ...
  for (const domain of targets) {
    ...
    const file = path.join(paths.domainImages, `${domain.id}.${config.format ?? 'webp'}`);
